...
class Mpu6050
{
  public:
    ...
    static constexpr uint8_t G_ST = 0x00U; // Gyro self test - not currently
                                           // used
    ...
    static constexpr uint8_t GYRO_CONFIG_250 =
        ((G_ST << 5U) | (FS_SEL_250 << 3U));
    static constexpr uint8_t GYRO_CONFIG_500 =
        ((G_ST << 5U) | (FS_SEL_500 << 3U));
    static constexpr uint8_t ACCEL_CONFIG =
        ((A_ST << 5U) | (AFS_SEL_16G << 3U));
  ...
  private:
    ...
    static_assert((Config::USE_250_DEGS_SECOND || Config::USE_500_DEGS_SECOND),
                  "You must set either USE_250_DEGS_SECOND or "
                  "USE_500_DEGS_SECOND in Config.hpp");
    static_assert((!Config::USE_250_DEGS_SECOND ||
                   !Config::USE_500_DEGS_SECOND),
                  "Only set USE_250_DEGS_SECOND or USE_500_DEGS_SECOND not "
                  "both in Config.hpp");
};