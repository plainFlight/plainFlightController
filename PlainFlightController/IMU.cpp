 ...
 *
 * This file is part of the PlainFlightController distribution
...
/**
 * @brief    Initialises MPU6050 as used by the IMU class and checks it is
 * communicating.
 */
void IMU::begin()
{
    ...
    {
        Serial.println("Failed to initialise MPU6050 at I2C address 0x68. "
                       "Check your wiring.");
        Serial.println("Alternatively change MPU6050_ADD in Mpu6050.hpp to "
                       "0x69 and try again.");
        m_imu.fault = true;
    }
    else
 ...
 */
bool IMU::isFaulted() { return m_imu.fault; }
 ...
 */
void IMU::setMadgwickWeighting(float weight) { m_bMadgwick = weight; }

/**
 * @brief  Attitude estimation through sensor fusion - 6DOF
 * @param
 * @note   See description of Madgwick() for more information.
 * @note   This is a 6DOF implimentation for when magnetometer data is not
 * available (for example when using the recommended MPU6050 IMU for the default
 * setup).
...
void IMU::Madgwick6DOF(const DemandProcessor::FlightState *const flightState)
{
    ...
    if (!((m_imu.mpu6050.accel_X == 0.0f) && (m_imu.mpu6050.accel_Y == 0.0f) &&
          (m_imu.mpu6050.accel_Z == 0.0f)))
    {
        // Normalise accelerometer measurement
        recipNorm = invSqrt((m_imu.mpu6050.accel_X * m_imu.mpu6050.accel_X) +
                            (m_imu.mpu6050.accel_Y * m_imu.mpu6050.accel_Y) +
                            (m_imu.mpu6050.accel_Z * m_imu.mpu6050.accel_Z));
        ...
        s1 = (_4q1 * q3q3) - (_2q3 * accelX) + (4.0f * q0q0 * m_q1) -
             (_2q0 * accelY) - _4q1 + (_8q1 * q1q1) + (_8q1 * q2q2) +
             (_4q1 * accelZ);
        s2 = (4.0f * q0q0 * m_q2) + (_2q0 * accelX) + (_4q2 * q3q3) -
             (_2q3 * accelY) - _4q2 + (_8q2 * q1q1) + (_8q2 * q2q2) +
             (_4q2 * accelZ);
        s3 = (4.0f * q1q1 * m_q3) - (_2q1 * accelX) + (4.0f * q2q2 * m_q3) -
             (_2q2 * accelY);
    ...
    recipNorm =
        invSqrt((m_q0 * m_q0) + (m_q1 * m_q1) + (m_q2 * m_q2) + (m_q3 * m_q3));
    m_q0 *= recipNorm;
    m_q1 *= recipNorm;
    m_q2 *= recipNorm;
    m_q3 *= recipNorm;
    ...
    {
        m_imu.pitch = -fastAtan2(((m_q1 * m_q3) - (m_q0 * m_q2)),
                                 0.5f - ((m_q1 * m_q1) + (m_q2 * m_q2))) *
                      57.29577951f;
        ...
        const float rads =
            constrain(2.0f * ((m_q0 * m_q1) + (m_q2 * m_q3)), -0.999999f,
                      0.999999f); // Prevent 'not a number' NaN.
    ...
    {
        m_imu.roll = fastAtan2((m_q0 * m_q1) + (m_q2 * m_q3),
                               0.5f - (m_q1 * m_q1) - (m_q2 * m_q2)) *
                     57.29577951f;
        const float rads =
            constrain(-2.0f * ((m_q1 * m_q3) - (m_q0 * m_q2)), -0.999999f,
                      0.999999f); // Prevent 'not a number' NaN.
...
/**
 * @brief    Calibrates gyro to reduce offset and drift, called as part of power
 * on initialisation.
 * @note     Uses sum of variances to detect movement which will upset
 * calibration.
 * @note     If CALIBRATE_MAX_VARIANCE_THRESHOLD is set too low the craft may
 * not calibrate due to IMU noise.
 * @note     CALIBRATE_MAX_VARIANCE_THRESHOLD may need tuning to suit your
 * calibration environment or how noisy your MPU6050 is.
 */
bool IMU::calibrateGyro()
{
    ...
    const int32_t deltaX =
        (static_cast<int32_t>(m_imu.mpu6050.rawGyro_X) << Q16_SHIFT) -
        m_xGyroMean;
    m_xGyroMean += deltaX / static_cast<int32_t>(m_calCount);
    const int32_t delta2X =
        (static_cast<int32_t>(m_imu.mpu6050.rawGyro_X) << Q16_SHIFT) -
        m_xGyroMean;
    m_xGyroM2 +=
        (static_cast<int64_t>(deltaX) * static_cast<int64_t>(delta2X)) >>
        Q16_SHIFT;

    // ---- Y axis ----
    const int32_t deltaY =
        (static_cast<int32_t>(m_imu.mpu6050.rawGyro_Y) << Q16_SHIFT) -
        m_yGyroMean;
    m_yGyroMean += deltaY / static_cast<int32_t>(m_calCount);
    const int32_t delta2Y =
        (static_cast<int32_t>(m_imu.mpu6050.rawGyro_Y) << Q16_SHIFT) -
        m_yGyroMean;
    m_yGyroM2 +=
        (static_cast<int64_t>(deltaY) * static_cast<int64_t>(delta2Y)) >>
        Q16_SHIFT;

    // ---- Z axis ----
    const int32_t deltaZ =
        (static_cast<int32_t>(m_imu.mpu6050.rawGyro_Z) << Q16_SHIFT) -
        m_zGyroMean;
    m_zGyroMean += deltaZ / static_cast<int32_t>(m_calCount);
    const int32_t delta2Z =
        (static_cast<int32_t>(m_imu.mpu6050.rawGyro_Z) << Q16_SHIFT) -
        m_zGyroMean;
    m_zGyroM2 +=
        (static_cast<int64_t>(deltaZ) * static_cast<int64_t>(delta2Z)) >>
        Q16_SHIFT;
    ...
    if (m_calCount >= CALIBRATE_MIN_SAMPLES_FOR_VARIANCE_CHECK)
    {
        ...
        if (varianceScaled >
            (static_cast<int64_t>(CALIBRATE_MAX_VARIANCE_THRESHOLD)
             << Q16_SHIFT))
        {
            if constexpr (Config::DEBUG_GYRO_CALIBRATION)
            {
    ...
    if (m_calCount >= CALIBRATE_COUNTS)
    {
        ...
        if constexpr (Config::DEBUG_GYRO_CALIBRATION)
        {
            ...
            Serial.print("x: ");
            Serial.print(m_imu.mpu6050.gyroOffset_X);
            Serial.print("\ty: ");
            Serial.print(m_imu.mpu6050.gyroOffset_Y);
            Serial.print("\tz: ");
            Serial.println(m_imu.mpu6050.gyroOffset_Z);
        }
 ...
 */
bool IMU::calibrated() { return m_imu.calibrated; }

/**
 * @brief    Main method of imu that sequences tasks.
 */
void IMU::operate(const float tDelta,
                  const DemandProcessor::FlightState *const flightState)
{
 ...
 */
IMU::ImuData *const IMU::getImuData() { return &m_imu; }
